...
def get_indices_from_one(elements: list[int]) -> list[int]:
    ...
    """
    return [x + 1 for x in range(len(elements))]
...
def get_max_element_index(elements: list[int]) -> int | None:
    ...
    """
    return None if not elements else max(range(len(elements)), key=lambda i: elements[i])
...
def get_first_three_index(elements: list[int]) -> int | None:
    ...
    """
    return next((i for i, x in enumerate(elements) if x == 3), None)
...
def get_last_three_index(elements: list[int]) -> int | None:
    ...
    """
    return next((len(elements) - i - 1 for i, x in enumerate(elements[::-1]) if x == 3), None)
...
def get_min_max(elements: list[int], default: int | None) -> tuple[int | None, int | None]:
    ...
    """
    return (default, default) if not elements else (min(elements), max(elements))
